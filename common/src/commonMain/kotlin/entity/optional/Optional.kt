...
public sealed class Optional<out T> {
    ...
    public data object Missing : Optional<Nothing>() {
        ...
        @Deprecated(
            message = "'Missing' was migrated to kotlin object. No need to create instance anymore",
            replaceWith = ReplaceWith("Missing"),
            level = DeprecationLevel.WARNING,
        )
        public operator fun invoke(): Missing = this
    }
    ...
    public data object Null : Optional<Nothing>() {
        ...
        @Deprecated(
            message = "'Null' was migrated to kotlin object. No need to create instance anymore",
            replaceWith = ReplaceWith("Null"),
            level = DeprecationLevel.WARNING,
        )
        public operator fun invoke(): Null = this
    }
     ...
     */
    public data class Value<T : Any>(override val value: T) : Optional<T>() {
        override fun toString(): String = "Optional.Value(content=$value)"
    }

    public companion object {

        public fun <T, C : Collection<T>> missingOnEmpty(value: C): Optional<C> =
            if (value.isEmpty()) Missing else Value(value)
