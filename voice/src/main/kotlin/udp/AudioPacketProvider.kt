...
public abstract class AudioPacketProvider(public val key: ByteArray, @Suppress("DEPRECATION") public val nonceStrategy: NonceStrategy) {
...
public class DefaultAudioPacketProvider(key: ByteArray, nonceStrategy: @Suppress("DEPRECATION") NonceStrategy) :
    AudioPacketProvider(key, nonceStrategy) {
    private val codec = XChaCha20Poly1305Codec(key)
    ...
    private val rtpHeaderView: ByteArrayView = packetBuffer.view(0, RTP_BASE_HEADER_LENGTH)!!

    private val nonceBuffer: MutableByteArrayCursor = ByteArray(nonceStrategy.nonceLength).mutableCursor()
    ...
    override fun provide(sequence: UShort, timestamp: UInt, ssrc: UInt, data: ByteArray): ByteArrayView =
        synchronized(lock) {
            with(packetBufferCursor) {
                ...
                resize(RTP_BASE_HEADER_LENGTH + (data.size + TweetNaclFast.SecretBox.boxzerobytesLength) + nonceStrategy.nonceLength)
                ...
                val encrypted = codec.encrypt(data, aad = packetBuffer, aadLength = RTP_BASE_HEADER_LENGTH, nonce = nonceBuffer.data, output = this)
