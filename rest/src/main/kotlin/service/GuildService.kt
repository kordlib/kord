...
class GuildService(requestHandler: RequestHandler) : RestService(requestHandler) {
    ...
    suspend inline fun createGuild(
        name: String,
        ...
        return call(Route.GuildPost) {
            body(
                GuildCreateRequest.serializer(),
                GuildCreateBuilder(name).apply(builder).toRequest()
            )
        }
    }
    ...
    @OptIn(ExperimentalContracts::class)
    suspend inline fun modifyGuild(
        guildId: Snowflake,
        builder: GuildModifyBuilder.() -> Unit
    ): DiscordGuild {
        contract {
            callsInPlace(builder, InvocationKind.EXACTLY_ONCE)
        }
     ...
     */
    @KordExperimental
    suspend fun getGuildMembers(guildId: Snowflake, query: String, limit: Int = 1) =
        call(Route.GuildMembersSearchGet) {
            keys[Route.GuildId] = guildId
    ...
    suspend fun addRoleToGuildMember(
        guildId: Snowflake,
        userId: Snowflake,
        roleId: Snowflake,
        reason: String? = null
    ...
    @OptIn(ExperimentalContracts::class)
    suspend inline fun addGuildBan(
        guildId: Snowflake,
        userId: Snowflake,
    ...
    @OptIn(ExperimentalContracts::class)
    suspend inline fun createGuildRole(
        guildId: Snowflake,
    ...
    suspend fun getGuildPruneCount(guildId: Snowflake, days: Int = 7) =
    ...
    suspend fun createGuildIntegration(
        guildId: Snowflake,
        integration: GuildIntegrationCreateRequest
    ...
    suspend inline fun modifyGuildIntegration(
        ...
        body(
            GuildIntegrationModifyRequest.serializer(),
            IntegrationModifyBuilder().apply(builder).toRequest()
        )
    }

    suspend fun deleteGuildIntegration(guildId: Snowflake, integrationId: Snowflake) =
    ...
    @Deprecated(
        ...
        ReplaceWith("getGuildWidget(guildId)"),
        DeprecationLevel.ERROR
    )
    suspend fun getGuildEmbed(guildId: Snowflake): Nothing =
    ...
    suspend fun getGuildWidget(guildId: Snowflake): DiscordGuildWidget =
        call(Route.GuildWidgetGet) {
            keys[Route.GuildId] = guildId
        }
    ...
    suspend fun getMembershipScreening(guildId: Snowflake): DiscordMembershipScreening =
        call(Route.GuildMemberVerificationGet) {
            keys[Route.GuildId] = guildId
        }

    suspend fun modifyMembershipScreening(
        guildId: Snowflake,
        request: MembershipScreeningModifyRequest
    ) =
        call(Route.GuildMemberVerificationPatch) {
            keys[Route.GuildId] = guildId
            body(MembershipScreeningModifyRequest.serializer(), request)
        }
}
