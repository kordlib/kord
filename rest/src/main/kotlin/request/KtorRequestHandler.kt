...
class KtorRequestHandler(
    ...
    private val logger =
        KotlinLogging.logger("[R]:[KTOR]:[${requestRateLimiter.javaClass.simpleName}]")
    ...
    override tailrec suspend fun <B : Any, R> handle(request: Request<B, R>): R {
        ...
        return when {
            ...
            response.isError -> {
                logger.debug { response.logString(body) }
                throw KtorRequestException(response,
    ...
    private suspend fun <B : Any, R> HttpClient.createRequest(request: Request<B, R>) =
